    ...
    <style>
        ...
        * { padding: 0; margin: 0; }
        @font-face {
            ...
            src: local("Source Sans Pro"), url("public/assets/fonts/calibri-font-family/calibri-regular.ttf") format("truetype");
            font-weight: normal;
            font-style: normal;
...
<div class="app" style="height: 11.693in; width: 8.268in; padding: 20px; background-color: white;">
    ...
    <div>
        <table class="table table-bordered p-0" style="border: 1px solid black !important; font-size: 15px;">
            ...
            <tr>
                ...
                <td>{{ $owner_name }}</td>
                <td><strong>مبایل شمیره: </strong> {{ $owner_phone }}</td>
            </tr>
            <!-- TIN & Export Date -->
            <tr>
                <th class="font-weight-bold">د <span style="font-family: 'Times New Roman';">TIN</span> شمیره</th>
                <td>{{ $company_tin }}</td>
                <td><strong>د صادریدو  نیټه: </strong> {{ \Morilog\Jalali\CalendarUtils::strftime('Y-m-d', strtotime($export_date)) }}</td>
            </tr>
            <!-- License Number & Expire Date -->
            <tr>
                <th class="font-weight-bold">د جواز نمبر</th>
                <td>{{ $license_number }}</td>
                <td><strong>د ختمیدو  نیټه: </strong> {{ \Morilog\Jalali\CalendarUtils::strftime('Y-m-d', strtotime($expire_date)) }}</td>
            </tr>
            <!-- Company Address -->
            <tr>
            ...
            <tr>
                ...
                <td colspan="2">د SMS (لنډ پیغام) د لاسته راوړلو لپاره شمیره:  {{ $phone }}</td>
            </tr>
            <!-- Company Phone -->
            <tr>
                <th colspan="3"><strong>بریښنالیک: </strong> {{ $email }}</th>
            </tr>
            </tbody>
        </table>
    </div>
    ...
    <!-- Verification -->
    <div>
        <p style="font-size: 16px;">د پورتنی معلوماتو تصدیق او د غوښتونکی لاسلیک او د شرکت ټاپه: -------------------------------------------------- نیته: {{ \Morilog\Jalali\CalendarUtils::strftime('Y-m-d', strtotime($created_at)) }}</p>
    </div>
